...
// Students may add or remove or edit fields, methods, constructors for this
// class
// Students may inherit from an use this class in any way internally in other
// classes.
...
//
// package level access means that all classes in the package can access
// directly.
// and accessing the node fields from classes other than your balanced search
// is bad design as it creates many more chances for bugs to be introduced and
// not
// caught.
//
...
class BSTNode<K, V> {
  ...
  private K getKey(BSTNode<K, V> node) {
    return node.key;
  }
  
  

  private void setLeft(BSTNode<K, V> node) {
    this.left = node;
  }

  private BSTNode<K, V> getLeft(K key) {
    return this.left;
  }

  private void setRight(BSTNode<K, V> node) {
    this.right = node;
  }

  private BSTNode<K, V> getRight(K key) {
    return this.right;
  }
